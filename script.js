...
// Contabiliza as respostas de cada curso (livre)
let courseCount = {};
// Contabiliza as respostas de cada área/curso superior (field)
let fieldCount = {};
...
function nextQuestion(e) {
    ...
    if (course !== undefined && course !== null) {
        console.log("Curso livre escolhido:", course);

        // Adiciona a resposta ao array de respostas
        responses.push(course);

        // Aumento da contagem do curso livre
    ...
    if (field !== undefined && field !== null) {
        console.log("Área/Curso Superior escolhido:", field);

        // Aumento da contagem da área/curso superior
...
function finish() {
    ...
    // Ordena os cursos LIVRES pela quantidade de respostas
    let sortedCourses = Object.entries(courseCount).sort((a, b) => b[1] - a[1]);

    // Exibe o resultado final CURSOS LIVRES
    textFinish.innerHTML = "RESPOSTA ENVIADA<br>Seu ranking de preferência de acordo com o teste:<br><br>";    // Mensagem de introdução

    // Exibe os 3 cursos LIVRES mais escolhidos com porcentagem
    sortedCourses.slice(0, 3).forEach(([course, count], index) => {
        let percentage = ((count / totalResponses) * 100).toFixed(2);        // Porcentagem calculada
        textFinish.innerHTML += `${index + 1}. Curso Livre: ${course} (${percentage}%)<br>`;
    });

    // Ordena as ÁREAS/CURSOS SUPERIORES (field) pela quantidade de respostas
    let sortedFields = Object.entries(fieldCount).sort((a, b) => b[1] - a[1]);

    textFinish.innerHTML += "<br><br>Seu ranking de Cursos Superiores (Áreas de Interesse):<br><br>";

    sortedFields.slice(0, 3).forEach(([field, count], index) => {
        let percentage = ((count / totalResponses) * 100).toFixed(2);
        textFinish.innerHTML += `${index + 1}. Área: ${field} (${percentage}%)<br>`;
    });
    ...
    <br><br>
    Sua opinião é fundamental para melhorar nossos serviços; por favor, preencha o formulário de feedback ao final.
    <br>
...
function loadQuestion() {
    ...
    item.answers.forEach((answer) => {
        ...
        if (answer.field) {
            button.setAttribute("data-field", answer.field);
        }
